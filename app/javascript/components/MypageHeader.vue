<template>
  <div class="w-full h-48 bg-violet-500 sticky top-20 z-10">
      ...
      <div class="grid grid-cols-3">
        <div class="icon sm:-pt-6 lg:-mt-12 pb-5 flex flex-col justify-center items-center">
          <div v-if="!user.icon_url">
            <img alt="icon" class="icon-image lg:mt-12 border-white rounded-full" src="../../../public/img/default_icon.jpg">
          </div>
          <div v-else>
            <img :src="user.icon_url" class="rounded-full icon-image border-white lg:mt-12"/>
          </div>
        ...
        <div class="bg-violet-100 h-full -mt-3 mr-1 px-2 py-3 rounded-lg">
          <p class="page-font mb-1 text-xs lg:text-xl"><span class="page-font">{{ user.name }}</span>の推し：</p>
          ...
          <div class="flex flex-col justify-between">
            <p class="page-font text-xs lg:text-xl" v-if="mygenreCharacters.length === 0">登録したジャンルを選んでください。</p>
            <ul v-for="mygenreCharacter in mygenreCharacters" class="rounded">
              <li class="page-font text-base lg:text-xl">{{ mygenreCharacter.character.name }}</li>
            </ul>
          </div>
        </div>
      </div>
